...
/*
 * Put Functions (Only sets if different than current value)
 */

void RTCZero::putSeconds(uint8_t seconds)
{
	if (_configured) {
...
void RTCZero::putMinutes(uint8_t minutes)
{
	if (_configured) {
...
void RTCZero::putHours(uint8_t hours)
{
	if (_configured) {
		if(getHours() != hours) {
			setHours(hours);
...
void RTCZero::putTime(uint8_t hours, uint8_t minutes, uint8_t seconds)
{
	if (_configured) {
		putSeconds(seconds);
		putMinutes(minutes);
...
void RTCZero::putDay(uint8_t day)
{
	if (_configured) {
		if(getDay() != day) {
			setDay(day);
...
void RTCZero::putMonth(uint8_t month)
{
	if (_configured) {
		if(getMonth() != month) {
			setMonth(month);
...
void RTCZero::putYear(uint8_t year)
{
	if (_configured) {
		if(getYear() != year) {
			setYear(year);
...
void RTCZero::putDate(uint8_t day, uint8_t month, uint8_t year)
{
	if (_configured) {
		putDay(day);
		putMonth(month);
...
void RTCZero::putAlarmSeconds(uint8_t seconds)
{
	if (_configured) {
		if(getAlarmSeconds() != seconds) {
			setAlarmSeconds(seconds);
		}
	}
}

void RTCZero::putAlarmMinutes(uint8_t minutes)
{
	if (_configured) {
		if(getAlarmMinutes() != minutes) {
			setAlarmMinutes(minutes);
		}
	}
}

void RTCZero::putAlarmHours(uint8_t hours)
{
	if (_configured) {
		if(getAlarmHours() != hours) {
			setAlarmHours(hours);
		}
	}
}

void RTCZero::putAlarmTime(uint8_t hours, uint8_t minutes, uint8_t seconds)
{
	if (_configured) {
		putAlarmSeconds(seconds);
		putAlarmMinutes(minutes);
		putAlarmHours(hours);
	}
}

void RTCZero::putAlarmDay(uint8_t day)
{
	if (_configured) {
		if(getAlarmDay() != day) {
			setAlarmDay(day);
...
void RTCZero::putAlarmMonth(uint8_t month)
{
	if (_configured) {
		if(getAlarmMonth() != month) {
			setAlarmMonth(month);
		}
	}
}

void RTCZero::putAlarmYear(uint8_t year)
{
	if (_configured) {
		if(getAlarmYear() != year) {
			setAlarmYear(year);
		}
	}
}

void RTCZero::putAlarmDate(uint8_t day, uint8_t month, uint8_t year)
{
	if (_configured) {
		putAlarmDay(day);
		putAlarmMonth(month);
...
void RTCZero::putEpoch(uint32_t ts)
{
	if (_configured) {
		if(getEpoch() != ts) {
			setEpoch(ts);
...
void RTCZero::putY2kEpoch(uint32_t ts)
{
	if (_configured) {
		if(getY2kEpoch() != ts) {
			setY2kEpoch(ts);
